...
import { ListEnd, Book, FlaskConical, FileText, Save, Loader2, TestTube2 } from "lucide-react";
import { useToast } from "@/hooks/use-toast";
import { apiRequest } from "@/lib/queryClient";
import LaboratoryTestConfirmation from "./laboratory-test-confirmation";
          ...
          <div className="space-y-4">
            {results.map((result, index) => (
              ...
              >
                ...
                {/* Detailed Symptom Matching Display */}
                {result.matchedSymptoms && result.matchedSymptoms.length > 0 && (
                  <div className="mb-3">
                    <div className="text-xs font-medium text-slate-700 mb-1">Matched Symptoms:</div>
                    <div className="flex flex-wrap gap-1">
                      {result.matchedSymptoms.slice(0, 5).map((symptom, index) => (
                        <span 
                          key={index}
                          className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                            symptom.frequency === 'Very frequent (99-80%)' || symptom.frequency === 'frequent' 
                              ? 'bg-green-100 text-green-800' 
                              : symptom.frequency === 'Frequent (79-30%)' || symptom.frequency === 'occasional'
                              ? 'bg-yellow-100 text-yellow-800'
                              : 'bg-gray-100 text-gray-800'
                          }`}
                          title={`${symptom.label} - ${symptom.frequency}`}
                        >
                          {symptom.label}
                          {symptom.frequency === 'Very frequent (99-80%)' && ' ⭐'}
                        </span>
                      ))}
                      {result.matchedSymptoms.length > 5 && (
                        <span className="text-xs text-slate-500">
                          +{result.matchedSymptoms.length - 5} more
                        </span>
                      )}
                    </div>
                  </div>
                ...
                <div className="flex space-x-2">
                  ...
                  <LaboratoryTestConfirmation
                    orphaCode={result.disease.orphaCode}
                    diseaseName={result.disease.name}
                    caseId={selectedCase?.disease.id === result.disease.id ? 1 : undefined}
                  />
                </div>
              </div>
            ))}
